...
t               <- 3e-1  # Temperature of heat bath
...
# myCreateNewFrame(file_name) # BE CAREFULL NOT TO ACCIDENTALLY OVERWRITE EXISTING FILE
...
rho             <- 0.05
# --------------------------------------------------------------------------------
x.beta <- myGetBetaValues(dim=dim, vol=nPart/rho, rho=rho)
# x.beta <- x.beta[which(x.beta>3e0)]

plt.data <- myGetObservables(dim=dim, vol=nPart/rho, rho=rho, beta=x.beta)
rm(x.beta)
