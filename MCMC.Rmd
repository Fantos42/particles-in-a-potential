...
nIt             <- 0.2e6  # Number of iterations
# nIt             <- 1
...
```{r}
nIt <- 2e5
x.ticks <- c(1e4:nIt)[which(c(1e4:nIt)%%1==0)]

# E1 <- MCMC_BurnIn_Run1$E
E2 <- MCMC_BurnIn_Run2$E
E3 <- MCMC_BurnIn_Run3$E

plot(NA, xlim=c(1,2e5), ylim=c(-2,2), xlab="Markov Chain Step", ylab="Energy")
# lines(x.ticks, E1[x.ticks], col="red")
lines(x.ticks, E2[x.ticks] - mean(E2[c((nIt-1e5):nIt)]), col="green")
