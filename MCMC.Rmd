...
# --------------------------------------------------------------------------------
# Betas To Calculate
x.beta <- exp(log(10)*seq(from=-7, to=7, by=0.2))
x.beta <- c(x.beta,1/exp(log(10)*seq(from=-2.1, to=-0.3, by=0.05))) # More detail in transition region
...
draw.DataWithError(x=x, y=y1, dy=y1err, col="black", bars=TRUE, points=TRUE, lines=TRUE) #q
...
draw.DataWithError(x=x, y=y4, dy=y4err, col="red"  , bars=TRUE, points=TRUE, lines=TRUE) #kappa

# abline(v=c(0.005,0.015,0.150,0.500), lwd=1, col="purple")



legend("topright", legend = c("q", expression(lambda), expression(kappa)), lty = c(1,1,1), col = c("black", "green", "red"))

#custom fit

#fit to q:
pars1 <- fit.automaticRoutine(startparam=c(5, 0.015, 0.1, 0.08), x=x, y=y1, dy=y1err, xrange=c(1e-3,1e3), fn=chisqr_PiecewiseLinAndExpFunction)
...
#fit to kappa:
pars4 <- fit.automaticRoutine(startparam=c(3, 0.005, 0.1, 0.5), x=x, y=y4, dy=y4err, xrange=c(1e-3,1e3), fn=chisqr_PiecewiseLinAndExpFunction)
fit.draw(pars4$par, x=x, col="red", lines=TRUE, vlines=FALSE)
