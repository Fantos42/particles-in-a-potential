...
nPart           <- 50
rho             <- 0.01
t               <- 0.2  # Temperature of heat bath
...
myPlotData(1/plt.data$x[which(plt.data$y.q<4)], plt.data$y.q[which(plt.data$y.q<4)]     , plt.data$y.q_Err     )
abline(h=1)
...
lines(x, y4, col="red"  , lwd=1)
arrows(x0=x, y0=y4-y4err, x1=x, y1=y4+y4err, angle=90, code=3, length=0.02, col="red")

# abline(v=1e-2)
# abline(v=0.005, lwd=1, col="purple")
# abline(v=0.015, lwd=1, col="purple")
# abline(v=0.150, lwd=1, col="purple")
# abline(v=0.500, lwd=1, col="purple")

# axis(1, at=axTicks(1), labels=paste(formatC(axTicks(1)/1000, format = 'd', 'kb')))

legend("topright", legend = c("q", expression(lambda), expression(omega), expression(kappa)), lty = c(1,1,1,1), col = c("black", "green", "blue", "red"))

#custom fit

fn <- function(par, x, y, dy) {
...
fitOrderParams <- function(par, x, y, dy){
  dy[which(dy==0)] <- 0.01
...
plotlin <- function(par, x){
  f <- (par[1] + (log(x) - log(par[2])) * (par[3]-par[1])/(log(par[4])-log(par[2])))
  return(f)
}

getErrorsOfFit <- function(par, x, y, dy){
  N <- length(y)
  dy[which(dy==0)] <- 0.01
  N_replicas <- 50
...
#fit to q:
id <- which(x > 1e-3 &  x < 1e3)
startparam1 <- c(3, 0.005, 0.1, 0.5)
par1    <- fitOrderParams(startparam1, x, y1, y1err)
par1err <- getErrorsOfFit(startparam1, x, y1, y1err)
# abline(h=par1$par[1], col="purple", lwd=2)
abline(v=par1$par[2], col="black", lwd=2)
# abline(h=par1$par[3], col="purple", lwd=2)
abline(v=par1$par[4], col="black", lwd=2)
# lines(x, plotlin(par1$par, x), col="purple", lwd=2)
cat("Fit to q: ")
cat("A1:    ", par1$par[1], " +- " , par1err$A1err, "\n")
cat("x1:    ", par1$par[2], " +- " , par1err$X1err, "\n")
cat("A2:    ", par1$par[3], " +- " , par1err$A2err, "\n")
cat("x2:    ", par1$par[4], " +- " , par1err$X2err, "\n")
cat("Chi^2: ", par1$chi2  , " +- " , par1err$chi2err,  " , p-value: ", par1$pval, "\n")

#fit to lambda:
id <- which(x > 1e-3 &  x < 1e3)
startparam2 <- c(0.5, 0.005, 3, 0.5)
par2    <- fitOrderParams(startparam2, x[id], y2[id], y2err[id])
par2err <- getErrorsOfFit(startparam2, x, y2, y2err)
# abline(h=par2$par[1], col="purple", lwd=2)
abline(v=par2$par[2], col="green", lwd=2)
# abline(h=par2$par[3], col="purple", lwd=2)
abline(v=par2$par[4], col="green", lwd=2)
# lines(x, plotlin(par2$par, x), col="purple", lwd=2)
cat("Fit to lambda: ")
cat("A1:    ", par2$par[1], " +- " , par2err$A1err, "\n")
cat("x1:    ", par2$par[2], " +- " , par2err$X1err, "\n")
cat("A2:    ", par2$par[3], " +- " , par2err$A2err, "\n")
cat("x2:    ", par2$par[4], " +- " , par2err$X2err, "\n")
cat("Chi^2: ", par2$chi2  , " +- " , par2err$chi2err,  " , p-value: ", par2$pval, "\n")

#fit to omega:
# id <- which(x > 1e-3 &  x < 1e3)
# par3 <- fitOrderParams(c(1, 0.005, 1, 0.42), x, y3, y3err)
# abline(h=par3$par[1], col="purple", lwd=2)
# abline(v=par3$par[2], col="blue", lwd=2)
# abline(h=par3$par[3], col="purple", lwd=2)
# abline(v=par3$par[4], col="blue", lwd=2)
# lines(x, plotlin(par3$par, x), col="purple", lwd=2)
# cat("Fit to omega: ", "A1: ", par3$par[1], "\n",  "x1: ", par3$par[2], "\n", "A2: ", par3$par[3], "\n", "x2: ", par3$par[4], "\n" ,
#          "Chi^2: ", par3$chi2, " , p-value: ", par3$pval, "\n")

#fit to kappa:
startparam4 <- c(3, 0.005, 0.1, 0.5)
par4    <- fitOrderParams(startparam4, x, y4, y4err)
par4err <- getErrorsOfFit(startparam4, x, y4, y4err)
# abline(h=par4$par[1], col="purple", lwd=2)
abline(v=par4$par[2], col="red", lwd=2)
# abline(h=par4$par[3], col="purple", lwd=2)
abline(v=par4$par[4], col="red", lwd=2)
# lines(x, plotlin(par4$par, x), col="purple", lwd=2)
cat("Fit to kappa: ")
cat("A1:    ", par4$par[1], " +- " , par4err$A1err, "\n")
cat("x1:    ", par4$par[2], " +- " , par4err$X1err, "\n")
cat("A2:    ", par4$par[3], " +- " , par4err$A2err, "\n")
cat("x2:    ", par4$par[4], " +- " , par4err$X2err, "\n")
cat("Chi^2: ", par4$chi2  , " +- " , par4err$chi2err,  " , p-value: ", par4$pval, "\n")
