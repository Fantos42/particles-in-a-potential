...
y4err[which(y4err==0)] <- 0.01
par1     <- c(3, 0.005, 0.1, 0.5)
par1 <- optim(par=par1, fn=fn, y=y4, dy=y4err, x=x)
print(par1$par)
abline(h=par1$par[1], col="red", lwd=2)
abline(v=par1$par[2], col="red", lwd=2)
abline(h=par1$par[3], col="red", lwd=2)
abline(v=par1$par[4], col="red", lwd=2)

lines(x, (par1$par[1] + (log(x) - log(par1$par[2])) * (par1$par[3]-par1$par[1])/(log(par1$par[4])-log(par1$par[2]))), col="purple", lwd=2)
