...
NumericVector timesTwo(NumericVector x) {
  Rcout << "Successfully entered" << std::endl;
  return x * 2;
}
```
```{r, echo=FALSE}
...
double stat_distr_quot(NumericMatrix y, NumericMatrix x, NumericVector q, int k){
  ...
  double a,b, sumx, sumy = 0;
  for(int i = 0; i < n; i++){
    if(i == (k-1)) continue;
    for(int j = 0; j < d; j++){
      sumx += (x((k-1),j) - x(i,j))*(x((k-1),j) - x(i,j));
      sumy += (y((k-1),j) - y(i,j))*(y((k-1),j) - y(i,j));
    }
    a += q[(k-1)] * q[i] / sqrt(sumy) - q[(k-1)] * q[i] / sqrt(sumx);
    b += 1/ pow(sumy, 4) - 1/ pow(sumx, 4);
    sumx = 0;
    sumy = 0;
  }
  return (a+b);
...
# start_time <- Sys.time()
Rlist1 <- random_walk_with_cpp(nIt, nPart, vol, t, X0, q)
# end_time <- Sys.time()
...
# y <- g(X0, k, vol)
# 
# piQuot <- pi_quot(y, X0, q, k, t)
# neupiquot <- stat_distr_quot(y, X0, q, k)
# 
# print(piQuot)
# print(neupiquot)
...
random_walk_with_cpp <- function(nIt, nPart, vol, t, X0, q) {
  ...
  for (n in c(2:nIt)) {
    # Generate Y_n
    k <- sample(c(1:nPart), 1)
    y <- getInstr_distr(X[(n-1),,], k, vol, sigma)
    # Set X_n
    DeltaE[n] <- stat_distr_quot(y, X[(n-1),,], q, k)
    if (runif(1) < exp(-1/t*DeltaE[n])) {
      X[n,,] <- y
      accept_rate <- accept_rate + 1
    } else {
